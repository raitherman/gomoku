...
public class Mängija {
	...
	public boolean kasMängijaVõitnud(int i, int j) {
		...
		return onVertikaalselt_5(hetkeveerg, j) || onHorisontaalselt_5(käigud[j], i) || onDiagonaal1_5(käigud, j, i)
	...
	public boolean onDiagonaal2_5(int[][] väljak, int x, int y) {
		...
		for (int i = -4; i < 1; i++) {
			if (x + i > -1 && y - i < Gomoku.mõõtmed && x + i + 4 < Gomoku.mõõtmed && y - i - 4 > -1) {
				if (kasVõrduvad(väljak[x + i][y - i], väljak[x + i + 1][y - i - 1], väljak[x + i + 2][y - i - 2],
						väljak[x + i + 3][y - i - 3], väljak[x + i + 4][y - i - 4])) {
					tõeväärtus = true;
				}
			}
		}
	...
	public boolean onHorisontaalselt_5(int[] rida, int mitmeselement) {
		...
		for (int i = -4; i < 1; i++) {
			if (mitmeselement + i > -1 && mitmeselement + i + 4 < Gomoku.mõõtmed) {
				if (kasVõrduvad(rida[mitmeselement + i], rida[mitmeselement + i + 1], rida[mitmeselement + i + 2],
						rida[mitmeselement + i + 3], rida[mitmeselement + i + 4])) {
					tõeväärtus = true;
				}
	...
	public boolean onVertikaalselt_5(int[] veerg, int mitmeselement) {
		...
		for (int i = -4; i < 1; i++) {
			if (mitmeselement + i > -1 && mitmeselement + i + 4 < Gomoku.mõõtmed) {
				if (kasVõrduvad(veerg[mitmeselement + i], veerg[mitmeselement + i + 1], veerg[mitmeselement + i + 2],
						veerg[mitmeselement + i + 3], veerg[mitmeselement + i + 4])) {
					tõeväärtus = true;
				}
			}
		}
