...
public class Gomoku extends Application {
    ...
    public class Lahter extends Pane {
		...
		public void setOmanik(Mängija omanik) {
        	this.omanik = omanik;
        	Pane p = new Pane();
    		File fail = new File("src/resources/" + (omanik.equals(punaneMängija) ? "punane.png" : "sinine.png"));
    		ImageView piltVaade = new ImageView(new Image(fail.toURI().toString(), this.getWidth(), this.getWidth(), true, true));
    		p.getChildren().add(piltVaade);
        ...
        private void teeKäik() {
            if (omanik == null) {
                ...
                if (aktiivneMängija.isKasArvuti()) {
                    ...
                    List<Koordinaadid> aktiivseMängijaKoordinaadid = aktiivneMängija.onHorisontaalselt_4();
                   	if (aktiivseMängijaKoordinaadid.size() != 0) {
                    	System.out.println(aktiivseMängijaKoordinaadid.get(0));
                    	System.out.println(väljad);
                    	väljad.get(aktiivseMängijaKoordinaadid.get(0)).teeKäik();
                    }
                }
            }
        }
